...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("nbuffer", "Choose buffer around your location (in km):", min = 1, max = 100, value = 100, step = 1),
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>% addTiles() %>% addCircleMarkers(data=sf_out(),radius= sf_out()$qplume) %>% 
