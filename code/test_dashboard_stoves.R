           ...
           fluidPage(
             sidebarLayout(
               sidebarPanel(width = 1,
                            ...
                            selectInput(inputId="filter",label="Select" ,choices = c("gas","Crude.Rate"),selected ="gas")
               ),
               mainPanel(
                 column(width = 4,
                 leafletOutput("map")
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet() %>% addTiles() %>% addPolygons(data=shp,fillColor= ~col(shp$gas),col="white",weight=1,popup = paste0(round(shp$gas*100,0),"% of households have gas stoves in ",shp$NAME)) %>%
